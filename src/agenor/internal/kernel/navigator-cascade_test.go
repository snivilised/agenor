	...
	DescribeTable("cascade",
		func(ctx SpecContext, entry *lab.CascadeTE) {
			...
			result, err := agenor.Walk().Configure().Extent(agenor.Prime(
				...
				agenor.IfOptionF(entry.Depth > 0, func() pref.Option {
					return agenor.WithDepth(entry.Depth)
				}),
				agenor.IfOptionF(entry.NoRecurse, func() pref.Option {
					return agenor.WithNoRecurse()
				}),
			)).Navigate(ctx)
