// i18n-gen is a code generator for go-i18n template data structs.
//
...
// # Installation
//
//	go install github.com/snivilised/i18n-gen@latest
//
// # Usage
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "i18n-gen: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	if *dryRun {
		fmt.Printf("i18n-gen: dry-run OK — no errors found, no files written (locale dir: '%s')\n", localeDir)
		return nil
	}
...
type underlierEntry struct {
	MessageID   string
	Seed        string
	TypeName    string // string form of the UnderlyingType constant
	Description string
	Story       string
	Other       string
	Fields      []fieldEntry
...
func extractEntry(cl *ast.CompositeLit) (underlierEntry, error) {
	...
	for _, elt := range cl.Elts {
		...
		case "TypeName":
			e.TypeName = identOrSel(kv.Value)
		case "Description":
...
func validate(entries []underlierEntry, verbose bool) error {
	...
	for _, e := range entries {
		...
		if ut == "UnderlyingTypeUndefined" || ut == "" {
	...
	sb.WriteString(fmt.Sprintf("i18n-gen: %d validation error(s) found — no files written:\n", len(errs)))
...
// is malformed — this is always a programmer error in i18n-gen.
...
func renderHeader(pkg string, imports []string) string {
	var sb strings.Builder
	sb.WriteString("// Code generated by i18n-gen. DO NOT EDIT.\n")
...
func emoji(ut string) string {
	switch {
	case strings.Contains(ut, "Cobra"):
		return "🧊"
	case strings.Contains(ut, "General"):
		return "📨"
	default:
		return "❌"
	}
