...
// executeRun builds and runs an agenor concurrent traversal using age.Hare,
...
func executeRun(ctx context.Context, inputs *RunInputs) error {
	...
	if inputs.WorkerPool.Native.CPU {
		opts = append(opts, age.WithCPU())
	} else if n := inputs.WorkerPool.Native.NoWorkers; n > 0 {
		opts = append(opts, age.WithNoW(uint(n)))
	}
	...
	result, err := age.Hare(isPrime, &wg)(facade, opts...).Navigate(ctx)
	wg.Wait()
