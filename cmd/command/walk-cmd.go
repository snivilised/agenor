...
// executeWalk builds and runs an agenor walk traversal using age.Tortoise,
...
func executeWalk(ctx context.Context, inputs *WalkInputs) error {
	...
	result, err := age.Tortoise(isPrime)(facade, opts...).Navigate(ctx)
	if err != nil {
...
func buildOptions(families SharedFamilies) []pref.Option {
	...
	if families.Cascade.Native.NoRecurse {
		opts = append(opts, age.WithNoRecurse())
	}

	if d := families.Cascade.Native.Depth; d > 0 {
		opts = append(opts, age.WithDepth(d))
	}

	// TODO: implement DryRun on age
	// if families.Preview.Native.DryRun {
	// 	opts = append(opts, age.WithDryRun())
	// }

	if families.Sampling.Native.IsSampling {
		opts = append(opts, age.WithSamplingOptions(&pref.SamplingOptions{
...
func resolveResumeStrategy(resume string) (age.ResumeStrategy, error) {
	switch resume {
	case ResumeStrategySpawn:
		return age.ResumeStrategySpawn, nil
	case ResumeStrategyFastward:
		return age.ResumeStrategyFastward, nil
	default:
		return 0, fmt.Errorf(
